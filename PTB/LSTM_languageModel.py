...
'''
num_steps与预处理后的句长相等
隐藏层单元个数hidden_size，循环步长num_steps，词向量维度embed_dim
三者间无必然联系

输入每个批次的句子原始维度为[batch_size, seq_len]
然后通过查表lookup函数查得每个词的词向量，此时句子维度变成[batch_size, seq_len, embed_dim]
RNN内部通过σ(Wx+b)将输入x转成输出h。W维度为[embed_dim, hidden_size]
由此，将输入维度embed_dim转化成了输出维度hidden_size。在展开为前向神经网络中，即为输入维度
一般情况，会把embedding与隐藏层状态设一致。

每一行数字代表原文中的一个句子，每个句子的结尾以0标记
截断句子长度为35
...
HIDDEN_SIZE = 300  # 隐藏层规模。也是词向量的维度
NUM_LAYERS = 2  # 深层循环神经网络中LSTM结构的层数。deep RNN
    ...
    def __init__(self, is_training, batch_size, num_steps):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(num_steps):
                if time_step > 0: tf.get_variable_scope().reuse_variables()# 对于一个句子的非第一个词，复用参数
                cell_output, state = cell(inputs[:, time_step, :], state)# 给出输入和上个时刻的状态，得到输出和当前时刻的状态
                outputs.append(cell_output) # 每一步的输出
        # 把输出队列展开成[batch, hidden_size*num_steps]的形状，
        # 然后再reshape成[batch*numsteps, hidden_size]的形状。
        ...
        logits = tf.matmul(output, weight) + bias  # 全连接层

        # 定义交叉熵损失函数和平均损失。
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            # shape为-1表示自动计算，最多只能有一个维度为-1
            # 这里只有一个维度，自动计算其大小
...
def run_epoch(session, model, batches, train_op, output_log, step):
    ...
    for x, y in batches:
        ...
        # cost, state, _ 与run的[model.cost, model.final_state, train_op]对应

        # 验证：run_epoch(session, eval_model, eval_batches,tf.no_op(), False, 0)
...
def make_batches(id_list, batch_size, num_step):
    ...
    data_batches = np.split(data, num_batches, axis=1) # [num_batches,batch_size,num_step]
    # 重复上述操作，但是每个位置向右移动一位。这里得到的是RNN每一步输出，即所需要预测的下一个单词。
    label = np.array(id_list[1: num_batches * batch_size * num_step + 1])
    label = np.reshape(label, [batch_size, num_batches * num_step])
    label_batches = np.split(label, num_batches, axis=1)  # [num_batches,batch_size,num_step]
